Feature: Update task status
  ...
  Background:
    ...
    Given The following tasks exist:
      | user                    | description                      | due_date   | estimated_duration | weight | state       |
  ...
  Scenario Outline: Successfully update the task status (normal flow)
    ...
    When The user attempts to update the status of the task "<description>" to "<new_state>"
    Then the task "<description>" shall be updated to "<new_state>"
    And "<user>" shall have a task called "<description>" with due date "<due_date>", duration "<estimated_duration>", weight "<weight>", and state "<new_state>"
    And The message "Task updated succesfully." shall be displayed

    Examples:
      | user                    | description                      | due_date   | estimated_duration | weight | new_state   |
  ...
  Scenario Outline: Update task with invalid status (error flow)
    Given "<user>" is logged in
    When The user attempts to update the status of the task "<description>" to "<new_state>"
    Then "<user>" shall have a task called "<description>" with due date "<due_date>", duration "<estimated_duration>", weight "<weight>", and state "<old_state>"
    And The error message "<error>" shall be displayed

    Examples:
      | user                    | description                      | due_date   | estimated_duration | weight | old_state   | new_state                 | error |
  ...
  Scenario Outline: Attempt to update a task without being logged in (error flow)
    ...
    And "<user>" shall have a task called "<description>" with due date "<due_date>", duration "<estimated_duration>", weight "<weight>", and state "<old_state>"
    And The user shall be at the login page

    Examples:
      | user                    | description                      | due_date   | estimated_duration | weight | old_state   | new_state   |
