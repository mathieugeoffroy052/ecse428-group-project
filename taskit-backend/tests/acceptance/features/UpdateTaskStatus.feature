Feature: Update task status
  ...
  Scenario Outline: Successfully update the task status (normal flow)
    ...
    And "<email>" shall have a task called "<name>" with due date "<due_date>", duration "<estimated_duration>", weight "<weight>", and state "<new_state>"
    And The message "Task updated succesfully." shall be displayed

    Examples:
      | email                    | name                             | due_date   | estimated_duration | weight | new_state   |
  ...
  Scenario Outline: Attempt to update a task without being logged in (error flow)
    ...
    Examples:
      | email                    | name                             | due_date   | estimated_duration | weight | old_state   | new_state   |
