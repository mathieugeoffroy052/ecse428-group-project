Feature: Create task
  ...
  Scenario: Successfully list all tasks (normal flow)
  ...
  Scenario: Unsuccessfully list all tasks (normal flow)
    Given "<username>" is logged in
    Given user "<username>" has tasks "<task_names>", Then the view function will not return this list of tasks
