Feature: Create task
  ...
  Background:
    ...
    Given The following tasks exist:
      | email                    | task_name                        | due_date   | estimated_duration | weight | state       | notes                               |
      | obi-wan.kenobi@gar.gov   | Train Anakin                     | 2022-02-06 | 1576800            | 90     | Not started | NULL                                |
      | anakin.skywalker@gar.gov | See through the lies of the Jedi | 2022-02-07 | 30                 | 75     | In progress | The Jedi were plotting to take over |
      | anakin.skywalker@gar.gov | Kill the younglings              | 2022-02-07 | 5                  | 25     | Complete    | Don't tell Padme                    |
  ...
  Scenario Outline: Successfully edit task filters (normal flow)
    ...
    Then "<email>" shall have a task called "<name>" with due date "<due_date>", duration "<estimated_duration>", weight "<weight>", state "Not started", and notes "<notes>"
  ...
  Scenario Outline: Attempt to create task without being logged in (error flow)
    ...
    When The user attempts to create the task of "<email>" called "<name>", due date "<due_date>", duration "<estimated_duration>", weight "<weight>", and notes "<notes>"
    ...
    Examples:
      | email                  | name            | due_date   | estimated_duration | weight | notes      |
      | obi-wan.kenobi@gar.gov | Die a sad death | 2022-02-08 | 30                 | NULL   | Run, Luke! |
