...
def step_impl(context,task_name,new_state):
    ...
    try:
        ...
        context.response = context.client.put(reverse("update_state",kwargs={"pk": task_id}), {"state":task_status})
...
def step_impl(context, task_name, new_state):
    ...
    task_status_str = str(task_status) if task_status is not None else ""
    print(task.state)
    assert_that(task.state, equal_to(str(task_status_str)), f'Unable to update to {new_state} state')
...
def step_impl(context,email,task_name,due_date,estimated_duration,weight,new_state):
    ...
    if(context.response.data == "Invalid task state"):
        assert_that(task.state, is_not(equal_to(task_status_str)), f'Could not update to state {new_state}')
    assert_that(task.state, (equal_to(task_status_str)), f'Could not update to state {new_state}')
