...
def when_the_user_attempts_to_create_the_task_list(context, email, list_name):
    request_data = {"list_name": list_name if list_name is not None else ""}
    try:
...
@when('The user attempts to create the task list of "{email}" called "{list_name}"')
def step_impl(context, email, list_name):
    when_the_user_attempts_to_create_the_task_list(context, email, list_name)


@then('the task list "{list_name}" shall exist in the system')
def step_impl(_, list_name):
    task_list = TaskList.objects.filter(list_name=list_name).first()
    assert_that(task_list, not_none())


@then('"{email}" shall have a task list called "{list_name}"')
def step_impl(_, email, list_name):
    owner = User.objects.filter(email=email).first()
    print(f"Task lists: {TaskList.objects.filter(list_name=list_name, owner=owner)}")
    task_list = TaskList.objects.filter(list_name=list_name, owner=owner).first()
    assert_that(task_list, not_none())


@then('shall not be a list called "{list_name:opt_?}"')
def step_impl(_, list_name):
    task_list = TaskList.objects.filter(list_name=list_name).first()
